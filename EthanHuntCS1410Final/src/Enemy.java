import java.awt.Graphics;
import java.awt.image.BufferedImage;

/*
Name: Ethan Hunt
Class: CS 1410-02
Assignment: Final
Date: 12/9/2019
*/

public class Enemy extends MovingObject{
	
	private int health;
	private int strength;
	private int pathPos;
	private int pathDistance;
	
	public Enemy(int posx, int posy, BufferedImage bi, int health, int strength) {
		super(posx, posy, bi, 38, 38, 2, 2);
		this.health = health;
		this.strength = strength;
		this.pathPos = -1;
		this.pathDistance = 0;
	}
	
	public void drawImage(Graphics g, int xDir, int yDir){
		//draws the image but with corner to right side 
		g.drawImage(bi, xDir - 38, yDir, imageW, imageH, null);
	}
	
	public void moveImage(Graphics g, Path[] path) {
		//vars
		int move[] = new int[2];
		int start[] = new int[2];
		
		//change position of enemy on path
		if(pathDistance == 0) {
			pathPos++;
			
			//get start
			start = path[pathPos].start();
			
			//assign start x and y
			posx = start[0];
			posy = start[1];
		}
		
		//get move var
		move = path[pathPos].move(pathDistance);
		
		//get current position of path array the corner is moved to top right
		drawImage(g, posx+=move[0], posy+=move[1]);
		System.out.println(posx + " " + posy);
		//increase path distance reset after 25
		pathDistance++;
		if(pathDistance >= 25) {
			pathDistance = 0;
		}
	}

}
